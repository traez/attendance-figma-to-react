...
export default function Medium() {
  ...
  return (
    <div className="flex flex-col items-center pb-8 bg-white rounded-2x">
      <aside className="justify-center px-4 py-1.5 text-sm font-medium tracking-normal leading-4 text-right text-white whitespace-nowrap bg-teal-600 rounded-t-xl">
      RECOMMENDED
    </aside>
      <section className="flex flex-col items-center pb-8 bg-white rounded-2xl border-t-4 border-solid border-teal-600">
        <article className="flex flex-row justify-between self-stretch p-8 w-full min-h-[144px]">
          <menu className="flex justify-center items-center p-5 w-20 h-20 bg-teal-50 rounded-lg">
        ...
        <article className="flex flex-col self-stretch px-8 w-full text-base">
          ...
          <div className="mt-2 leading-6 text-slate-600">
            Medium membership base and also want to carry out other actions. One
            time $50 startup and configuration fee. 5% per transaction
          </div>
          <div className="flex flex-row gap-1 justify-center px-4 py-2 mt-2 text-teal-600 hover:text-white hover:bg-teal-600 rounded-lg border-2 border-teal-600 border-solid w-3/5 cursor-pointer">
            <div className="font-bold text-xs">Get Started</div>
            <div className="relative h-4 w-10 my-auto aspect-square">
              <Image
                src="/icon-arrow-right-teal.svg"
                alt=""
                fill
                sizes="(min-width: 400px) 100vw"
              />
            ...
            <menu key={index} className="flex gap-3 px-5 mt-4 w-full">
              ...
              <div
                className={`text-base leading-6 text-slate-600 ${
                  index >= websiteFeatures.length - 2
                    ? "line-through text-opacity-30"
                    : ""
                }`}
              >
                {feature}
